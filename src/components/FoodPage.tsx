...
function FoodPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const data = response.data as { value: FoodItem[] };
        setFoods(data.value);
      } catch (error) {
  ...
  const handleAddFood = async (data: FoodItem) => {
    ...
    try {
      ...
      const newFood = response as FoodItem;
      console.log(newFood);
      setFoods([...foods, newFood]); // Update local state
      setNewFoodName("");
                    ...
                    <FormControl fullWidth>
                      <TextField
                        {...register("calories", {
                          ...
                          validate: (value) =>
                            value > 0 || "Calories must be positive.",
                        })}
